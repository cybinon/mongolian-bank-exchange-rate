# 🏦 Монголын Банкуудын Валютын Ханш API

Монголын 13 банкны валютын ханшийг цуглуулж, API-аар үйлчилдэг нээлттэй эхийн төсөл.

## ✨ Онцлогууд

- 📊 Монголын 13 томоохон банкны валютын ханш
- 🔄 Өдөр бүр автоматаар шинэчлэгдэнэ
- 📅 Өмнөх өдрүүдийн ханшийг хайх боломжтой
- 🏦 Банкаар шүүж хайх
- 🚀 FastAPI дээр суурилсан API
- 🐳 Docker дээр ажиллана

## 🏛️ Дэмжигдсэн Банкууд

| Банкны нэр | Код |
|------------|-----|
| Ариг Банк | ArigBank |
| Богд Банк | BogdBank |
| Капитрон Банк | CapitronBank |
| Чингис Хаан Банк | CKBank |
| Голомт Банк | GolomtBank |
| Хаан Банк | KhanBank |
| М Банк | MBank |
| Монгол Банк | MongolBank |
| Үндэсний Хөрөнгө Оруулалтын Банк | NIBank |
| Төрийн Банк | StateBank |
| Худалдаа Хөгжлийн Банк | TDBM |
| Транс Банк | TransBank |
| Хас Банк | XacBank |

## 🌐 API Эцсийн Цэгүүд

| Эцсийн цэг | Тайлбар |
|------------|---------|
| `GET /` | API-ийн мэдээлэл |
| `GET /health` | status шалгах |
| `GET /rates` | Бүх ханшийг авах |
| `GET /rates/latest` | Бүх банкны хамгийн сүүлийн ханш |
| `GET /rates/bank/{bank_name}` | Тодорхой банкны ханш |
| `GET /rates/date/{date}` | Тодорхой өдрийн ханш |
| `GET /rates/bank/{bank_name}/date/{date}` | Тодорхой банк, өдрийн ханш |

## � Docker ашиглан ажиллуулах

```bash
# Репо clone хийх
git clone https://github.com/btseee/mongolian-bank-exchange-rate.git
cd mongolian-bank-exchange-rate

# Docker Compose ашиглан эхлүүлэх
docker-compose up -d

# API http://localhost:8000 дээр ажиллана
# Docs http://localhost:8000/docs дээр харагдана
```
## 💻 Локал орчинд ажиллуулах

```bash
# Репо clone хийх
git clone https://github.com/btseee/mongolian-bank-exchange-rate.git
cd mongolian-bank-exchange-rate

# Virtual environment үүсгэх
python -m venv .venv

# Идэвхжүүлэх (Windows)
.venv\Scripts\activate

# Идэвхжүүлэх (Linux/Mac)
source .venv/bin/activate

# Хамаарлуудыг суулгах
pip install -r requirements.txt

# Playwright browser суулгах
playwright install chromium

# API эхлүүлэх
uvicorn app.api.main:app --reload

# Cron ажиллуулах (өөр терминалд)
python cron.py
```
## 🔧 Орчны хувьсагчууд

| Хувьсагч | Анхдагч утга | Тайлбар |
|----------|--------------|---------|
| `DATABASE_URL` | `sqlite:///./exchange_rates.db` | Өгөгдлийн сангийн холболт |
| `CRON_SCHEDULE` | `0 9 * * *` | Cron хуваарь (UTC+8) |
| `SSL_VERIFY` | `true` | SSL баталгаажуулалт |
| `PORT` | `8000` | API порт |

## 📝 Лиценз

MIT License

## 🤝 Хувь нэмэр оруулах

Pull request болон issue нээхийг урьж байна!
